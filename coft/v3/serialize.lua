...
-- const
local SERDES_DEBUG_ENABLED = false--if shared.SERDES_DEBUG_ENABLED then true else false
...
return function(serializable: Instance): string
	...
	lookup.dynamicClasses(f.ClassName)
		...
		for i = 1, serializeGroupCount do
			...
			do
				...
				for i, obj: Instance in serializeGroup do
					...
					lookup.dynamicClasses(obj.ClassName) -- insert class name into dynamic classes if its not in ther
	...
	graph:writeblocks(compression.Zlib.Compress(dynamicdata, {level=9, strategy="dynamic"}))
	...
	do
		...
		for obj: Instance, index: number in pairs(lookup.instanceIds.entries) do
			...
			instanceidgraph:writeu16(lookup.dynamicClasses(obj.ClassName))
		end
		
		instanceIdData = instanceidgraph:export()
	end
	graph:writeblocks(compression.Deflate.Compress(instanceIdData, {level=9, strategy="dynamic"}))	
	...
	for i, propertyData in propertyDatas do
		...
		graph:writeblocks(compression.Deflate.Compress(propertyData, {level=9, strategy="dynamic"}))
	end
